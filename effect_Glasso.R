...
for(l in 1:4){
  ...
  for(k in 1:10){
    ...
    for(j in 1:traitnum){
      ...
      y <- Xb + error
      YY[,j]=y}
    lambda<-lsgl::lambda(x1,YY, alpha=0, lambda.min=0.05)
    fit<-lsgl::fit(x = x1, y = YY, alpha = 0, lambda = lambda)
  ...
  store[[l]]=perform
  write.table(perform,paste0('/home/unfated/simulation/powerspectrum/effectcor_GLasso-h',l,'.txt'))
}