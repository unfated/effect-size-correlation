...
colnames(pval)<-'x'
qqplot(-log10(qunif(seq(0,1,length.out = nrow(pval)))),-log10(pval$x),xlab="expected -log10(p-value)",ylab="observed -log10(p-value)",main='qq plot')
qqline(pval$x,col='red',distribution = qunif)
qqplot(qunif(seq(0,1,length.out = 10)),pval$x,xlab="null p-value",ylab="observed p-value",main='qq plot')
qqline(pval$x,col='red',distribution = qunif)
# png(paste0('distplot_cutoff',i,'.png'))
# print(p)
# dev.off()
#get genotype
# real data
...
useddata$y2<-y
  ...
  write.table(pval,paste0('/home/unfated/pval',herit,"_",prop[r1],'.txt'))
...
write.table(perform,paste0('/home/unfated/perform',herit,"_",prop[r1],'.txt'))
