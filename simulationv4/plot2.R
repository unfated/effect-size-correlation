...
p5 <- cowplot::plot_grid(fig[[1]],fig[[2]],fig[[3]],fig[[4]],fig[[5]],fig[[6]],fig[[7]],fig[[8]],fig[[9]], nrow = 3, labels = LETTERS[1:10])#将p1-p5四幅图组合成一幅图，按照两行两列排列，标签分别为A、B、C、D。（LETTERS[1:4] 意为提取26个大写英文字母的前四个:A、B、C、D）
...
fig<-list()
for(k in 1:10){
perform1<-as.data.frame(matrix(1,9,5))
...
p2<-ggplot(data=test_data_long2, aes(x=lambda, y=value, colour=variable)) + geom_line()+geom_point()+geom_vline(xintercept = lambdaused)
...
# 
p5
png(width=1700,height=1000,paste0('ridge_0814_differlambda_',herit,'.png'))
print(p5)
dev.off()
p5
