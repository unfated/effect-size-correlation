...
for(j in 1:5){
  ...
  perform <- read.csv(paste0("C:/Users/22367/Desktop/result/perform0815",h2[j],'_',prop[i],".txt"), sep="")
  ...
  p3<-p2+xlab('cutoff:-log10(pval)')+ggtitle(paste0('performance when P=(h2=',h2[j],',lambda=',round(lambdaused,4), ',NO.causal=',m.causal[i],',perSNP h2=',round(h2[j]/m.causal[i],4),")"))
  fig[[j]]=p3
}

p5 <- cowplot::plot_grid(fig[[1]],fig[[2]],fig[[3]],fig[[4]],fig[[5]],nrow = 3, labels = LETTERS[1:10])#将p1-p5四幅图组合成一幅图，按照两行两列排列，标签分别为A、B、C、D。（LETTERS[1:4] 意为提取26个大写英文字母的前四个:A、B、C、D）

p5
png(width=1500,height=1000,paste0('ridge_cutoff0814_',h2[j],'_',prop[i],'.png'))
print(p5)
dev.off()
p5
}
