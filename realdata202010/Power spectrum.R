...
effect1=cbind(effect,data.frame(matrix(0,nrow=100,ncol=p-n.causal)))
TN=0
TP=0
FP=0
FN=0
...
FDR=FP/(FP+TP)
power=1-FNR
perform[[i]]=c(FPR,FNR,FDR,power,(h1+h2)/(2*n.causal),n.causal,prop[i])
#length(which(BETA[,1]==0))
#length(which(BETA[,1]!=0))
print(paste0('i=',i))
}
write.table(perform,'/home/unfated/simulation/uniLasso-h4.txt')