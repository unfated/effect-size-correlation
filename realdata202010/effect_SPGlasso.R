...
for(l in 1:4){
  ...
  for(k in 1:10){
    ...
    for(j in 1:traitnum){
      ...
      y <- Xb + error
      YY[,j]=y}
    lambda<-lsgl::lambda(x1,YY, alpha=0.5, lambda.min=0.05)
    fit<-lsgl::fit(x = x1, y = YY, alpha = 0.5, lambda = lambda)
    betalasso=as.matrix(coef(fit, traitnum)[,-1])
    ff=lsgl::features(fit)[[traitnum]][-1]
    
    # Betalasso=data.frame(as.matrix(fit$beta))
  ...
  store[[l]]=perform
  write.table(perform,paste0('/home/unfated/simulation/powerspectrum/effectcor_SPGLasso-h',l,'.txt'))
}